...
function ListProcessor(resource) {
    ...
    this._elements = resource._elements.map(a => ({
        ...a
    })).map(a => a._instanciate_processor());
    ...
    this._add = async function() {
        ...
        for(elementIx in args)
        {
          ...
          if (typeof(element) == "string")
              element = await resource._instanciate_processor()._lookup(element);
    ...
    this._lookup = async function(identifier, setOwner) {
        ...
        for (element in resource._elements) {
            result = await resource._elements[element]._instanciate_processor()._in_lookup(ref, true);
            if (result)
                return result;
        }
